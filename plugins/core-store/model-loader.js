...
function find(cache, base, type, appSpecializationName) {
...
export function lookup(store, type, appSpecializationName) {
  ...
  try {
    return find(cache, require(`@/plugins/app-extension/${ store }/models/${ type }`), type, appSpecializationName);
  } catch (e) {
  }

  try {
    return find(cache, require(`@/models/${ store }/${ type }`), `${ store }/${ type }`.appSpecializationName);
  } catch (e) {
  }

  try {
    return find(cache, require(`@/models/${ type }`), type, appSpecializationName);
  } catch (e) {
  }
